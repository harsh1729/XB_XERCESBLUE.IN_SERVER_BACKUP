	...
	public function index($page=false)
	{
        ...
        //if($this->session->userdata('web_language') == 'hi')
         //   $this->session->set_userdata('web_language',NULL);
            ...
            $bankpo['gkpdf'] = $this->Bankpo_model->get_all_pdf_CurrentGk(2,0);
		...
		else if($is_logged_in)
		{
			if(!$page)
			{
	            ...
	            $bankpo['status'] = $this->Blog_model->get_all_status_data($page);
				//$data = $this->Bankpo_model->get_all_set($item_id,$userid);
				$bankpo['gkpdf'] = $this->Bankpo_model->get_all_pdf_CurrentGk(2,$userid);
				$this->load->view('Bankpo-view',$bankpo);
...
public	function importantpdf()
	...
	{   
		...
		if($id == 0)
		{
			if($this->input->get_post('lang_type') !== null )
			{
				/*&& $this->input->Get('price_type') !== null*/
				$lang_type=json_decode($this->input->get_post('lang_type'));
				//$price_type=json_decode($this->input->Get('price_type'));
				$isparent = $this->input->get_post('isparrent');
				$catid = 0;
				...
				$type = 6;
				$userid = $this->Bankpo_model->get_session_userid();
				$video['category'] = $this->Bankpo_model->get_allcate();
				//price type is hardcodede free
				/*$video['data'] = $this->video_model->get_all_video($type,$userid,$lang_type,$price_type,$catid);*/
				$video['lang_type'] = $lang_type;
				$video['catid'] = $catid;

               if($isparent==1){
				    $javasc['parrentid'] = $catid;
				    $video['data'] = $this->video_model->get_all_video($type,$userid,$lang_type,$price_type,0,$catid,$page);
                 }else{
                 	 $video['data'] = $this->video_model->get_all_video($type,$userid,$lang_type,$price_type,$catid,0,$page);
                 	if($catid!=0)
				  $javasc['parrentid'] = $this->Bankpo_model->getparrentcatid($catid);
				 else
            	   $javasc['parrentid'] = 0;
                 }
                ...
                $footerdata['js'] = $this->load->view('view-parts/video-view-js-files',$javasc,TRUE);
			    $this->load->view('view-parts/footer',$footerdata);
			}
			else
			{
		...
		else
		{
			...
			else if($is_logged_in)
			{
			   $catid = $this->input->get_post('catid');
				...
				$type = 6;
				$userid = $this->Bankpo_model->get_session_userid();
				$video['data'] = $this->video_model->show_single_video($userid,$type,$id,$catid);
...
public function current_affaires($date = false, $name= false)
	{
				...
				if(!$date && !$name)
				{
					...
					/*$data['last_update_date'] = $this->Bankpo_model->get_last_update_date();*/
				...
				else if($date != "" && !$name)
				{
					...
					if(strlen($date) <= 11)
					{
						...
						if($index == '1')
						{
							...
							/*$data['current_affaire_current_month'] =$this->Bankpo_model->get_last_update_date();
							$data['current_update_gk'] = $this->Bankpo_model->get_gk_by_month($monthdate); */
						...
						else if($index == '2')
						{
							...
							$data['current_affaire_quiz_current_month'] =date("Y-m");
							/*$data['current_affaire_last_month'] = $this->Bankpo_model->get_last_update_date();*/
