...
$XamDetailQuery = "select * from XamAlert_Details_Test";
...
while($XamDetailRow = mysqli_fetch_array($XamDetailResult) )
{
	...
	$title = $root->appendChild($title);
			...
			$ExamOpeningDate->value = $XamDetailRow['OpeningDate'];
			$title->appendChild($ExamOpeningDate);
			
			$DetailString = "";
       //   $OfflineLastDate  = .$XamDetailRow['OfflineLastDate'].;
            $DetailString .= " .$XamDetailRow['Name']."
			...
			$XamPostsQuery = "select * from XamAlert_Posts_Test where XamDetailId=". $XamDetailRow['Id'];
			...
			while($XamPostsRow = mysqli_fetch_array($XamPostsResult) )
			{
				...
				$DetailString .= "@@Examination : ";
				
				$XamDatesQuery = "select * from XamAlert_Dates_Test where PostId=".$XamPostsRow['Id'];
