...
//$insertDownloadQuery = "insert into downloadcounter (AppId,DeviceId,LangCode) values (".$id_of_app.",".$id_of_device.",'".$langCode."')";
...
$qus_query = "select * from ( select * from qrecord as t2 where t2.QuesId in ( select t1.QuesId from qusappmapping as t1 where t1.AppId = ".$id_of_app." ) ) as t3 where t3.QuesId > ".$id_of_last_qus;
...
while($qus_row=mysqli_fetch_array($qus_result))
{
		...
		$qus_opt = $doc->createElement('Option');
		...
		if($qus_row['Solution'] != "")
		{
            ...
            $answer_text_node = $doc->createTextNode($qus_row['Solution']);
